...
rename_vars <- function(dat, report_missing = FALSE) {
  # no merge_queries because only one query possible so renaming of lists here
  # wrap in dataframes to have empty sets if NULL
  d <- list()
  d$main <- dat$data %>% purrr::flatten()
  d$users <- dat$includes$users %>% purrr::flatten()
  d$tweets <- dat$includes$tweets %>% purrr::flatten()
  d$media <- dat$includes$media %>% purrr::flatten()
  d$places <- dat$includes$places %>% purrr::flatten()
  # all$errors ?
  # all$polls ?
  # all$meta
...
fill_missing_tibbles <- function(d) {
  ...
  # this is the version for non query merging (e.g. single tweet lookup)
  # -> extra subtables need to be added
  if (is.null(d[["tweets"]])) {
    d$tweets <- data.frame()
...
preselect_vars <- function(d) {
  ...
  d$users <- d$users %>% select(-contains("entities"), -contains("name"), -contains("user_url"), -contains("user_protected"), -contains("user_profile_image"), -contains("user_pinned_status_id"))
  d$tweets <- d$tweets %>% select(-contains("entities"), -contains("ref_media_id")) # TODO: implement ref_media_id (nested)
